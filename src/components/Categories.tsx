...
function Categories() {
  ...
  return (
    <section className="flex flex-col max-sm:mt-6 mt-4">
        <section className="flex flex-wrap gap-y-2 gap-x-3 lg:px-12 px-10 max-sm:gap-x-2 max-sm:px-6">
            {allCategories?.map((item, index)=>(
                 
                  <button key={index} className="text-center w-20  lg:w-24 py-[0.1rem] px-2  rounded-2xl max-sm:px-1 max-sm:w-22 bg-slate-400" onClick={()=> getCategory(item)}>{item}</button>        
            ))}
        </section>
        <section>
